  ...
  return (
    <main className="h-screen relative">
      <section className="flex flex-col items-center gap-2 flex-1 h-full">
        <div className="grid grid-cols-[1fr_1fr] h-full  w-full justify-around text-gray-700">
          ...
          <span className="absolute left-1/2 top-4 text-6xl -translate-x-1/2">¿Qué deseas Añadir?</span>
          <span className="absolute left-1/2 bottom-80 text-6xl -translate-x-1/2 pointer-events-none">O</span>
            {session.user.role == "ADMIN" && (
            <Link href="create/coffee_shop" className="text-3xl flex flex-col border-t-2 border-gray-600 justify-center items-center p-4 transition-colors hover:bg-black/5">
              <FaCoffee className="w-32 h-32 text-gray-700" /> Una Cafeteria 
            </Link>
          )}
        </div>
